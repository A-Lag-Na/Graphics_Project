...
bool Particles::Initialize(ID3D11Device* device)
{
	bool result;
...
bool Particles::Frame(float frameTime, ID3D11DeviceContext* deviceContext)
{
	bool result;
...
void Particles::Render(ID3D11DeviceContext* deviceContext, ID3D11VertexShader* vertexShader, ID3D11PixelShader* pixelShader, ID3D11InputLayout* inputLayout, ID3D11RenderTargetView* view, ID3D11ShaderResourceView* SRV = nullptr, ID3D11SamplerState* sampler = nullptr)
{
...
int Particles::GetIndexCount()
{
	return m_indexCount;
}


bool Particles::InitializeParticleSystem()
{
	int i;
...
void Particles::ShutdownParticleSystem()
{
...
bool Particles::InitializeBuffers(ID3D11Device* device)
{
	...
	m_vertices = new SimpleVertex[m_vertexCount];
	if (!m_vertices)
	{
		return false;
	}
	...
	memset(m_vertices, 0, (sizeof(SimpleVertex) * m_vertexCount));
...
void Particles::ShutdownBuffers()
{
...
void Particles::EmitParticles(float frameTime)
{
...
void Particles::UpdateParticles(float frameTime)
{
	int i;
...
bool Particles::UpdateBuffers(ID3D11DeviceContext* deviceContext)
{
	int index, i;
	HRESULT result;
	...
	{
		// Bottom left.
		m_vertices[index].Pos = XMFLOAT3(m_particleList[i].positionX - m_particleSize, m_particleList[i].positionY - m_particleSize, m_particleList[i].positionZ);
		m_vertices[index].Tex = XMFLOAT2(0.0f, 1.0f);
		m_vertices[index].Normal = XMFLOAT3(m_particleList[i].red, m_particleList[i].green, m_particleList[i].blue);
		index++;
	...
	memcpy(verticesPtr, (void*)m_vertices, (sizeof(SimpleVertex) * m_vertexCount));
...
void Particles::RenderBuffers(ID3D11DeviceContext* deviceContext, ID3D11VertexShader* vertexShader, ID3D11PixelShader* pixelShader, ID3D11InputLayout* inputLayout, ID3D11RenderTargetView* view, ID3D11ShaderResourceView* SRV = nullptr, ID3D11SamplerState* sampler = nullptr)
{
	unsigned int stride;
	unsigned int offset;
